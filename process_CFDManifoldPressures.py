...
p_manifold = np.asarray([manifold_data[3 * i + 2] for i in range(n_manifolds)])
...
p_manifold_function = []
p_manifold_junction = np.zeros((n_manifolds, z_junction.shape[0]))
p_junction_in = np.zeros((n_manifolds, z_junction.shape[0]))
p_junction_out = np.zeros((n_manifolds, z_junction.shape[0]))
dp_junction = np.zeros((n_manifolds, z_junction.shape[0]))
...
manifold_mass_flows = np.zeros((n_manifolds, channel_mass_flows.shape[0] + 1))
...
for i in range(n_manifolds):
    zeta_junction[i] = \
         ...
         * manifold_flow_direction_z[i]) / (v1[i] ** 2.0)
...
z_channel = np.asarray([channel_data[3 * i + 1] for i in range(n_channels)])
p_channel = np.asarray([channel_data[3 * i + 2] for i in range(n_channels)])
lin_coeffs = \
...
p_channel_linear_in = \
    [poly(y_channel_in, lin_coeffs[i]) for i in range(n_channels)]
y_channel_out = y_channel[-1] + manifold_height * 0.5
p_channel_linear_out = \
    [poly(y_channel_out, lin_coeffs[i]) for i in range(n_channels)]
